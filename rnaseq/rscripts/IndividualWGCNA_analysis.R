...
mMods <- blockwiseModules(exp$maledata$data, checkMissingData = FALSE, maxBlockSize = 50000, corType = "bicor",
...
male_module.dist <- as.data.frame(table(mMods$colors) %>% sort(decreasing = TRUE))
colnames(male_module.dist) <- c("Module", "Genes")
